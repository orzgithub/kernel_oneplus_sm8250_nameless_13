...
static struct z_erofs_pcluster_slab pcluster_pool[] __read_mostly = {
	_PCLP(1),  _PCLP(4),   _PCLP(16),
	_PCLP(64), _PCLP(128), _PCLP(Z_EROFS_PCLUSTER_MAX_PAGES)
};
...
static inline int z_erofs_init_workqueue(void)
{
	 ...
	 */
	z_erofs_workqueue =
		alloc_workqueue("erofs_unzipd", WQ_UNBOUND | WQ_HIGHPRI,
...
static int z_erofs_attach_page(struct z_erofs_collector *clt, struct page *page,
			       enum z_erofs_page_type type, bool pvec_safereuse)
{
	...
	ret = z_erofs_pagevec_enqueue(&clt->vector, page, type, pvec_safereuse);
...
static void z_erofs_try_to_claim_pcluster(struct z_erofs_collector *clt)
{
	...
	if (cmpxchg(&pcl->next, Z_EROFS_PCLUSTER_NIL, *owned_head) ==
	 ...
	 */
	if (cmpxchg(&pcl->next, Z_EROFS_PCLUSTER_TAIL, *owned_head) ==
...
hitted:
	...
	page_type =
		cur ? Z_EROFS_VLE_PAGE_TYPE_HEAD :
				       ...
				       struct list_head *pagepool)
{
	...
	err = z_erofs_decompress(
		&(struct z_erofs_decompress_req){ .sb = sb,
						  .in = compressed_pages,
						  .out = pages,
...
jobqueue_init(struct super_block *sb, struct z_erofs_decompressqueue *fgq,
	      bool *fg)
{
		...
		do {
			...
			if (!bio) {
				...
				bio->bi_iter.bi_sector =
					(sector_t)cur << LOG_SECTORS_PER_BLOCK;
