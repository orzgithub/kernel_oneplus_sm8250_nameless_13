...
void _erofs_err(struct super_block *sb, const char *function, const char *fmt,
		...)
{
...
void _erofs_info(struct super_block *sb, const char *function, const char *fmt,
		 ...)
{
	struct va_format vaf;
	va_list args;
				 ...
				 struct erofs_super_block *dsb)
{
	...
	if (sbi->available_compr_algs & ~Z_EROFS_ALL_COMPR_ALGS) {
		erofs_err(
			sb,
			"try to load compressed fs with unsupported algorithms %x",
				 ...
				 struct erofs_super_block *dsb)
{
	if (dsb->u1.available_compr_algs) {
		erofs_err(
			sb,
			"try to load compressed fs when compression is disabled");
		return -EINVAL;
	}
	return 0;
}
...
#ifdef CONFIG_EROFS_FS_ZIP
static int erofs_build_cache_strategy(struct super_block *sb, substring_t *args)
{
...
static match_table_t erofs_tokens = { { Opt_user_xattr, "user_xattr" },
...
static int __init erofs_module_init(void)
{
	...
	erofs_inode_cachep =
		kmem_cache_create("erofs_inode", sizeof(struct erofs_inode), 0,
				  SLAB_RECLAIM_ACCOUNT, erofs_inode_init_once);
